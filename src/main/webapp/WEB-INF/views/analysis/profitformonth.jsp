...
<body>
<c:set var="menu" value="monthprofit" />
...
$(function() {
    ...
    function monthChartDraw(dayData) {
     ...
     function monthCircleChartDraw(dayData) {
	      ...
	      var options = {
	    	title: dayData[0].day + " ~ " + dayData[dayData.length-1].day + "매출 현황",
	    	slices: {  2: {offset: 0.2},
                		4: {offset: 0.3},
      		},
      		pieHole: 0.5,
	      ...
	      var chart = new google.visualization.PieChart(document.getElementById('monthCircleChart'));
	...
	function CategoryChartDraw(dayData) {
        ...
        var dataArr = [];
        $.each(dayData, function(index, data) {
              dataArr.push([data.category, data.profit]);
        });
        data.addRows(dataArr);
        var options = {
           title: "카테고리별 매출 현황",
