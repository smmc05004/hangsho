...
<c:set var="dealInfo" value="saleB"></c:set>
		...
		<table class="table table-condensed">
			...
			<tbody>
				<c:if test="${empty sales }">
					<tr>
						<td colspan="6" class="text-center">등록한 판매글이 없습니다.</td>
					</tr>
				</c:if>
				<c:forEach var="sale" items="${sales }">
					<tr>
						...
						<c:choose>
							<c:when test="${sale.type.id eq 'M' }">
								<td><a href="/merona/detail.do?no=${sale.id }"><c:out value="${sale.title }" /></a></td>
							</c:when>
							<c:otherwise>
								<td><a href="/used/detail.do?no=${sale.id }"><c:out value="${sale.title }" /></a></td>
							</c:otherwise>
						</c:choose>
						<td>${sale.status.name }</td>
						<td><fmt:formatDate value="${sale.createdate }" pattern="yyyy-MM-dd" /></td>
						<td><button type="button" class="btn btn-danger btn-xs" data-toggle="modal" data-target="#call" id="btn-request-${sale.id }">${sale.requestCnt }건</button></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
	...
	<div class="modal fade" id="call" role="dialog">
		<div class="modal-dialog">
			...
			<div class="modal-content">
				...
				<form action="/approveRequest.do" method="post" id="approveRequest">
...
<script>
	$(function() {
		$('.pagination a').click(function(event) {
			...
			location.href = "deal.do?pageNo=" + $(this).attr('href');
		});
